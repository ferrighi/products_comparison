...
class ProductsComparisonController extends ControllerBase {
    ...
    public function render(){
      ...
      global $platforms, $firstyears_array, $fname_postf, $fname_sensor, $tiles_array;
      global $datadir, $datadir2;
        ...
        #define products
        $platforms = array("S2A", "S2B");

        #define first year of products
        $firstyears_array = "2015";
 ...
 public function getTitle() {
   ...
   return  $title;
    ...
    function get_date($year,$month,$day) {
      ...
      list($defyear, $defmonth, $defday) = $this->find_yesterday();
    ...
    public function getProductTime($tile) {
      ...
      foreach ($result as $doc) {
         ...
         //$cloud = isset($field['platform_ancillary_cloud_coverage']) ? $field['platform_ancillary_cloud_coverage'] : 'None';
         ...
         \Drupal::logger('comparison_module')->debug('Timestring '.$count .': ' . $date['year'] . '/' .$date['month'] . '/' . $date['day'] . ' - ' .$date['time']);
         //\Drupal::logger('comparison_module')->debug('Cloudcoverage '.$count .': ' . $cloud);
         $count = $count + 1;
      }
      rsort($prinfo);

    //  $time_array = [];
    //  $time_array['None'] = 'YYYY/MM/DD - THHMMSS';
    //  for ($y=1;$y<count($prinfo)+1;$y++) {
    //    $time_array[$prinfo[$y][1]] = $prinfo[$y][0][0]. '/' .$prinfo[$y][0][1].'/' .$prinfo[$y][0][2]. " - ".$prinfo[$y][0][3];
